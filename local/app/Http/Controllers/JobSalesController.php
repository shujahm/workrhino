...
class JobSalesController extends Controller
{
	...
	public function seller_cancel($chat_id,$order_id,$status)
	{
     ...
     if($status=="yes")
	 {
      ...
      $gig_order = DB::table('gig_order')
		                    ->where('id','=',$order_id)
				            ->get();
      $transfer_user_id = $gig_order[0]->user_id;
	  $amount = $gig_order[0]->price - $gig_order[0]->processing_fee;
	  
	  $get_user = DB::table('users')->where('id','=',$transfer_user_id)->get();
	  $old_balance = $get_user[0]->wallet;
	  
	  $final_amount = $old_balance + $amount;
	  DB::update('update users set wallet="'.$final_amount.'" where id = ?', [$transfer_user_id]);	
	...
	$star_rate = $data['star_rate'];
		...
		if(!empty($gig_orderr))
		{
			...
			$gig_order = DB::table('gig_order')
				            ...
				            ->get();
							$gid = $gig_order[0]->gid;
      $transfer_user_id = $gig_order[0]->gig_user_id;
	  $reference_id = $gig_order[0]->reference_id;
	  $amount = $gig_order[0]->price - $gig_order[0]->processing_fee;
	 ...
	 $url = URL::to("/");
		...
		Mail::send('payment_email', ['gid' => $gid, 'reference_id' => $reference_id, 'amount' => $amount,
		'site_logo' => $site_logo, 'site_name' => $site_name, 'url' => $url, 'currency' => $currency], function ($message) use ($adminemail)
        {
	 ...
	 Mail::send('payment_email', ['gid' => $gid, 'reference_id' => $reference_id, 'amount' => $amount,
		'site_logo' => $site_logo, 'site_name' => $site_name, 'url' => $url, 'currency' => $currency], function ($message) use ($adminemail,$email)
        {
            $message->subject('Payment received your wallet');
	...
	DB::update('update gig_order set amount_by="buyer",payment_level="4" where id = ?', [$order_id]);	
	 ...
	 $url = URL::to("/");
		...
		Mail::send('refund_email', ['gid' => $gid, 'reference_id' => $reference_id, 'amount' => $amount,
		'site_logo' => $site_logo, 'site_name' => $site_name, 'url' => $url, 'currency' => $currency], function ($message) use ($adminemail)
        {
	 ...
	 Mail::send('refund_email', ['gid' => $gid, 'reference_id' => $reference_id, 'amount' => $amount,
		'site_logo' => $site_logo, 'site_name' => $site_name, 'url' => $url, 'currency' => $currency], function ($message) use ($adminemail,$email)
        {
            $message->subject('Payment refund your wallet');
