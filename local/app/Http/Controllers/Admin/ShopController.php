...
class ShopController extends Controller
{
	...
	public function status_view($id,$sid)
	{
		
		DB::update('update shop set status="'.$sid.'" where id = ?', [$id]);
		return back();
	}
		...
		if($editid!="")
		{
			...
			status="'.$status.'",admin_email_status="'.$admin_email_status.'" where id = ?', [$editid]);
					...
					{	
				         if($status=="approved")
						{
						Mail::send('admin/shopmail', ['shop_name' => $shop_name, 'address' => $shop_address, 'shop_phone_no' => $shop_phone_no, 'description' => $shop_desc, 'site_logo' => $site_logo, 'site_name' => $site_name], function ($message)
					{
