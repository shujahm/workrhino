...
class EdituserController extends Controller
{
	...
	$settings = DB::select('select * from settings where id = ?',[1]);			 
      ...
      return view('admin.edituser',['users'=>$users, 'userid' => $userid, 'books' => $books, 'books_cnt' => $books_cnt, 'settings' => $settings]);
   }
    ...
    protected function edituserdata(Request $request)
    {
		...
		DB::update('update users set name="'.$name.'",email="'.$email.'",password="'.$passtxt.'",phone="'.$phone.'",photo="'.$savefname.'",admin="'.$admin.'",wallet="'.$wallet.'" where id = ?', [$id]);
